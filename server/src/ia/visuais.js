...
export const VISUAIS = {
  ...
  'relatorios:diario-vendas': {
    ...
    oQueE: 'Meta, realizado, projeção e as duas médias diárias de VENDA, em dois blocos de tecnologia: FIBRA por cidade e TELEFONIA só com contagem e média, porque não tem meta na origem. Rádio não entra: está zerado no recorte e foi retirado da tela. A meta por dia divide por dias produtivos (o mês inteiro, porque é alvo); a média por dia divide por dias úteis já decorridos. Sábado vale meio dia, domingo e feriado zero.',
    recorte: (p) => ({
  ...
  'relatorios:diario-ativos': {
    ...
    oQueE: 'O mesmo quadro para ATIVAÇÃO, em fibra. Conta pela data de ativação, não pela da venda — por isso não fecha com a tabela de vendas, e isso é proposital. ATENÇÃO: dois clientes institucionais são excluídos da contagem de ativação (estão em `clientesExcluidos`), porque valem um prédio inteiro e distorcem a conta; é o filtro que a origem aplica em cada visual de ativos.',
    recorte: (p) => ({
      periodo: p.periodo, dias: p.dias, origemDaMeta: p.metasOrigem,
  ...
  'relatorios:diario-fila': {
    ...
    oQueE: 'Instalações em aberto, com equipamento ainda em estoque. Não respeita o período: fila em aberto é retrato do agora. A divisão é por EQUIPE, e é ela que responde a pergunta operacional: BKO é o que está parado na conferência de cadastro (equipe Validação de dados), AGENDADOS é o que já tem agenda e espera a rua. Só fibra: rádio está zerado no recorte e saiu da tela.',
    recorte: (p) => ({

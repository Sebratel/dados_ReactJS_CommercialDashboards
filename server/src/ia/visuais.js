...
export const VISUAIS = {
  ...
  'relatorios:diario-vendas': {
    ...
    oQueE: 'Meta, realizado, projeção e as duas médias diárias de VENDA, em três blocos de tecnologia: FIBRA por cidade, RÁDIO com meta única (não por cidade, é assim na origem) e TELEFONIA só com contagem e média, porque não tem meta. A meta por dia divide por dias produtivos (o mês inteiro, porque é alvo); a média por dia divide por dias úteis já decorridos. Sábado vale meio dia, domingo e feriado zero.',
    recorte: (p) => ({
      periodo: p.periodo, dias: p.dias, origemDaMeta: p.metasOrigem,
      fibraPorCidade: p.fibra?.vendas?.linhas,
      fibraTotal: p.fibra?.vendas?.total,
      radio: p.radio?.vendas,
      telefoniaPorCidade: p.telefonia?.linhas,
      telefoniaTotal: p.telefonia?.total,
    }),
  },
  'relatorios:diario-ativos': {
    ...
    oQueE: 'O mesmo quadro para ATIVAÇÃO, em fibra e rádio. Conta pela data de ativação, não pela da venda — por isso não fecha com a tabela de vendas, e isso é proposital. ATENÇÃO: dois clientes institucionais são excluídos da contagem de ativação (estão em `clientesExcluidos`), porque valem um prédio inteiro e distorcem a conta; é o filtro que a origem aplica em cada visual de ativos.',
    recorte: (p) => ({
      periodo: p.periodo, dias: p.dias, origemDaMeta: p.metasOrigem,
      fibraPorCidade: p.fibra?.ativos?.linhas,
      fibraTotal: p.fibra?.ativos?.total,
      radio: p.radio?.ativos,
      valorInstaladoFibra: p.fibra?.cartoes?.valor,
      valorInstaladoRadio: p.radio?.cartoes?.valor,
      clientesExcluidos: p.excluidos,
    }),
  },
  'relatorios:diario-fila': {
    ...
    oQueE: 'Instalações em aberto, com equipamento ainda em estoque. Não respeita o período: fila em aberto é retrato do agora. A divisão é por EQUIPE, e é ela que responde a pergunta operacional: BKO é o que está parado na conferência de cadastro (equipe Validação de dados), AGENDADOS é o que já tem agenda e espera a rua. O segundo eixo é o tipo — fibra e rádio.',
    recorte: (p) => ({
      bkoFibra: p.fila?.bkoFibra,
      agendadosFibra: p.fila?.agendadosFibra,
      bkoRadio: p.fila?.bkoRadio,
      agendadosRadio: p.fila?.agendadosRadio,
